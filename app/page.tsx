...
const steps = [
 { number: '01', title: '대화에서 시작합니다', en: 'CONSULTATION', text: '생활 방식과 예산, 입주 일정을 듣습니다. 현장 실측 후 구조 변경의 범위와 공정별 견적을 함께 검토합니다.', duration: '상담 · 실측 / 1–2주' },
 { number: '02', title: '도면을 공간으로', en: 'DESIGN & 3D', text: '평면 계획부터 마감재 샘플, 조명 배치까지. 3D 공간으로 동선과 비례를 확인하고 실시설계를 확정합니다.', duration: '설계 · 소재 협의 / 4–6주' },
 { number: '03', title: '디테일을 현실로', en: 'CONSTRUCTION', text: '철거, 설비, 전기, 목공, 마감의 순서로 진행합니다. 현장 담당자가 주간 공정과 변경 사항을 공유합니다.', duration: '현장 시공 / 8–12주' },
 { number: '04', title: '완성 이후에도', en: 'AFTERCARE', text: '준공 체크리스트로 마감을 검수하고 사용 안내를 전달합니다. 입주 후 하자 접수와 정기 점검을 이어갑니다.', duration: '준공 검수 · A/S / 12개월' },
];
export default function Home() {
 ...
 return <main ref={root}>
  ...
  <section id="studio" className="studio section-wrap">
   <div className="section-label" data-reveal><span className="eyebrow">01 / OUR PHILOSOPHY</span><span className="label-caption">공간을 바라보는 우리의 시선</span></div>
   <div className="studio-main" data-reveal><h2>좋은 공간은<br />삶을 앞서가지 않습니다.<br /><span>삶에 자연스럽게 스며듭니다.</span></h2><div className="studio-bottom"><p>보여주기 위한 장식을 덜고, 매일 닿는 것에 집중합니다.<br />걷는 동선, 손이 닿는 소재, 빛이 머무는 자리.<br />당신의 생활을 이해하는 것에서 우리의 설계는 시작됩니다.</p><a className="text-link" href="#process">우리의 작업 방식 <span>↗</span></a></div></div>
   <div className="principles" data-reveal><div><span>01</span><h3>생활을 읽는 설계</h3><p>동선과 수납, 가족의 생활 리듬을<br />평면 계획에 담습니다.</p></div><div><span>02</span><h3>오래 남는 소재</h3><p>유행보다 질감과 내구성을.<br />실물 샘플로 마감을 결정합니다.</p></div><div><span>03</span><h3>한 팀의 책임 시공</h3><p>설계와 현장을 하나의 기준으로.<br />준공 이후의 관리까지 이어집니다.</p></div></div>
  </section>
  <section id="projects" className="portfolio section-wrap">
   <div className="section-top" data-reveal><div><span className="eyebrow">02 / SELECTED WORKS</span><h2 className="display-title">Selected <em>spaces.</em></h2></div><p>저마다의 일상, 그에 맞는 공간.<br />우리가 함께 완성한 세 개의 집.</p></div>
   <div className="project-grid">{projects.map((project, index) => <article className={`project project-${index + 1}`} key={project.id} data-reveal><button className="project-image" onClick={() => setSelectedProject(project)} aria-label={`${project.korean} 시공 사례 자세히 보기`}><Image src={project.image} alt={`${project.korean} — 밝은 갤러리 스타일 주거 인테리어`} fill sizes={index === 0 ? '(max-width: 700px) 100vw, 90vw' : '(max-width: 700px) 100vw, 50vw'} /><span className="project-view">프로젝트 보기 <span>↗</span></span></button><div className="project-info"><div><span className="eyebrow">{project.number} / {project.type}</span><h3><button onClick={() => setSelectedProject(project)}>{project.title}</button></h3><p>{project.korean}</p></div><div className="project-meta"><span>{project.location}</span><span>{project.area} <span className="meta-slash">/</span> {project.year}</span></div></div></article>)}</div>
   <p className="portfolio-note">본 프로젝트는 포트폴리오 시연을 위한 가상의 시공 사례입니다.</p>
  </section>
  <section id="showroom" className="showroom-section">
   <div className="section-wrap"><div className="section-top" data-reveal><div><span className="eyebrow">03 / SPATIAL EXPERIENCE</span><h2 className="display-title">Before it <em>becomes real.</em></h2></div><p>도면을 넘어, 공간 안으로.<br />시공 전의 설계를 세 가지 시점으로 만나보세요.</p></div>
