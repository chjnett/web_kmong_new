...
export default function Home() {
 ...
 return <main ref={root}>
  ...
  <section className="hero" id="top" aria-label="아틀리에 에어로 소개">
   <div className="hero-media"><video ref={video} autoPlay muted playsInline loop preload="auto" poster="/images/hero-poster.jpg" onCanPlay={() => { if (wantsPlayback.current) video.current?.play().catch(() => setPlaying(false)); }} onPlay={() => setPlaying(true)} onPause={() => setPlaying(false)} onError={() => setPlaying(false)} aria-label="거실의 설계가 완성되는 인테리어 영상"><source src="/videos/hero-interior.webm?v=3" type="video/webm" /><source src="/videos/hero-interior.mp4?v=3" type="video/mp4" /></video></div>
