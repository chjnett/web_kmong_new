...
export default function ContactDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
 ...
 return <dialog ref={ref} className="contact-dialog" onCancel={onClose} onClose={onClose} aria-labelledby="contact-title" onClick={(event) => { if (event.target === event.currentTarget) onClose(); }}>
  ...
  {sent ? <div className="success-state" role="status"><span className="eyebrow">THANK YOU</span><h2 id="contact-title">당신의 공간 이야기를<br />들려주셔서 감사합니다.</h2><p>견적 신청 시연이 완료되었습니다.<br />입력하신 정보는 저장되거나 전송되지 않습니다.</p><button className="button dark" onClick={onClose}>공간 더 둘러보기 <span>↗</span></button></div> : <>
   <span className="eyebrow">START YOUR PROJECT</span><h2 id="contact-title">새로운 공간의 시작.</h2><p className="dialog-lead">공간과 계획을 알려주세요. 상담에 필요한 항목만 담았습니다.</p>
   <form onSubmit={submit} className="contact-form">
    ...
    <label className="form-wide">요청 사항 <span className="optional">선택</span><textarea name="message" rows={3} maxLength={2000} placeholder="변경하고 싶은 구조, 선호하는 소재, 생활 방식 등을 자유롭게 남겨주세요." /></label>
