...
export default function ContactDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
 ...
 return <dialog ref={ref} className="contact-dialog" onCancel={onClose} onClose={onClose} aria-labelledby="contact-title" onClick={(event) => { if (event.target === event.currentTarget) onClose(); }}>
  ...
  {sent ? <div className="success-state" role="status"><span className="eyebrow">THANK YOU</span><h2 id="contact-title">상담 내용을<br />작성해 주셔서 감사합니다.</h2><p>견적 신청 시연이 완료되었습니다.<br />입력하신 정보는 저장되거나 전송되지 않습니다.</p><button className="button dark" onClick={onClose}>사이트로 돌아가기 <span>↗</span></button></div> : <>
   <span className="eyebrow">START YOUR PROJECT</span><h2 id="contact-title">리모델링 상담 신청</h2><p className="dialog-lead">시공 지역과 면적, 예산을 알려주세요. 희망 일정에 맞춰 계획을 정리하는 데 필요한 정보입니다.</p>
   <form onSubmit={submit} className="contact-form">
    ...
    <label className="form-wide">상담 내용 <span className="optional">선택</span><textarea name="message" rows={3} maxLength={2000} placeholder="바꾸고 싶은 구조나 선호하는 소재를 적어주세요. 가족의 생활 방식도 함께 알려주시면 좋습니다." /></label>
